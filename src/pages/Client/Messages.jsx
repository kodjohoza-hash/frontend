...
const MessagesPage = () => {
  ...
  const handleSelect = useCallback((convId) => { setActiveConvId(convId); setMobileShowChat(true); }, []);
  const handleBack = useCallback(() => { setMobileShowChat(false); }, []);

  const handleSend = useCallback((text) => {
    if (!activeConvId) return;
    const newMessage = { id: `msg_${Date.now()}`, senderId: currentUser.id, text, timestamp: new Date().toISOString(), status: 'sent' };
    setAllMessages((prev) => ({ ...prev, [activeConvId]: [...(prev[activeConvId] || []), newMessage] }));
    setTimeout(() => setAllMessages((prev) => ({ ...prev, [activeConvId]: prev[activeConvId].map((m) => m.id === newMessage.id ? { ...m, status: 'delivered' } : m) })), 1200);
    setTimeout(() => setAllMessages((prev) => ({ ...prev, [activeConvId]: prev[activeConvId].map((m) => m.id === newMessage.id ? { ...m, status: 'read' } : m) })), 3000);
  }, [activeConvId]);
  ...
  return (
    <div className="msg-layout">
      <div className={`msg-layout__sidebar ${mobileShowChat ? 'msg-layout__sidebar--hidden' : ''}`}>
        <MessageSidebar conversations={initialConversations} contacts={contacts} activeId={activeConvId} onSelect={handleSelect} />
      </div>
