    ...
    <Suspense fallback={<TkTicketSkeleton />}>
      ...
      <div className="tk-page__content">
        ...
        {filtered.length === 0 ? <TkTicketEmptyState /> : viewMode === 'grid' ? (
          <div className="tk-ticket-grid">{filtered.map((ticket) => <TkTicketCard key={ticket.id} ticket={ticket} onView={setPreviewTicket} />)}</div>
        ) : (
          <div className="tk-ticket-list">{filtered.map((ticket) => <TkTicketCard key={ticket.id} ticket={ticket} viewMode="list" onView={setPreviewTicket} />)}</div>
        )}
      </div>
      {previewTicket && <TkTicketPreviewModal ticket={previewTicket} onClose={() => setPreviewTicket(null)} />}
    </Suspense>
  );
};
